    ...
    match user_result {
        Ok(user) => {
            match user {
                Some(user) => {
                    let password_result =
                        UserCredentials::get_password_by_id(&mut db, &user.id).await;
                            ...
                            match verification_result {
                                Ok(true) => {
                                    ...
                                    let refresh_token_result =
                                        UserToken::db_select_by_user_id(&mut db, &user.id).await;
                                    ...
                                    match refresh_token_result {
                                        Ok(Some(_refresh_token)) => {
                                            ...
                                            match tx_result {
                                                Ok(mut tx) => {
                                                    let new_token = random_string::generate(
                                                        64,
                                                        random_string::charsets::ALPHANUMERIC,
                                                    );
                                                    let update_result =
                                                        UserToken::db_update_refresh_token(
                                                            &mut tx, &user.id, &new_token,
                                                        )
                                                        .await;
                                                    match update_result {
                                                        Ok(_) => {
                                        ...
                                        Ok(None) => {
                                            ...
                                            match tx_result {
                                                Ok(mut tx) => {
                                                    let new_refresh_token = random_string::generate(
                                                        64,
                                                        random_string::charsets::ALPHANUMERIC,
                                                    );
                                ...
                                Ok(false) => {
                                    ApiResponse::String(Status::Unauthorized, "Invalid credentials")
                                }
                                Err(err) => {
