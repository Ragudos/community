            ...
            match hash_result {
                Ok(hashed_password) => {
                    ...
                    match tx_result {
                        Ok(mut tx) => {
                            let user_result =
                                User::create(&mut tx, &register_data.display_name).await;

                            match user_result {
                                Ok(user) => {
                                    let user_id = &user.id;
                                    let user_metadata_result = UserMetadata::create(
                                        &mut tx,
                                        user_id,
                                    ...
                                    match user_metadata_result {
                                        Ok(_) => {
                                            let user_credentials_result = UserCredentials::create(
                                                &mut tx,
                                                user_id,
                                                &hashed_password,
                                            )
                                            ...
                                            match user_credentials_result {
                                                Ok(_) => {
                                                    ...
                                                    let refresh_token = random_string::generate(
                                                        64,
                                                        random_string::charsets::ALPHANUMERIC,
                                                    );
                                                    let store_token_result = UserToken::db_create(
                                                        &mut tx,
                                                        user_id,
                                                        &refresh_token,
                                                    )
