...
#[get("/?<list_query..>")]
pub fn page<'r>(cookie_jar: &CookieJar<'r>, user: UserJWT, is_boosted: IsBoosted, list_query: Option<ListQuery<'r>>) -> Template {
    let IsBoosted(is_boosted) = is_boosted;
    let theme = Theme::from_cookie_jar(cookie_jar);
    let metadata = SeoMetadata::build().theme(theme).finalize();

    Template::render("pages/community", context! { metadata, user, is_boosted, list_query })
}
