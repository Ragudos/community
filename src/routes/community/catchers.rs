...
pub async fn community_page_not_found_get(
    ...
    let user = request
        .guard::<UserJWT>()
        .await
        .expect("UserJWT guard must be valid in a 500 Internal Server Error catcher for communities.");
...
#[catch(401)]
pub fn community_page_unauthorized_get(
    request: &Request<'_>
) -> Redirect {
    let community_id = request.param::<i64>(0).unwrap().unwrap();
    let is_about_page = request.uri().to_string().contains(
        format!("/community/{}/about", community_id).as_str()
    );
    ...
    let is_delete_community_uri = request.uri().to_string().contains(
        format!("/community/{}/delete", community_id).as_str()
    );
