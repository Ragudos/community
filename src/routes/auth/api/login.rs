...
use crate::controllers::errors::login_error::{get_login_data_or_return_validation_error, render_error};
...
pub async fn post<'r>(
    ...
    let Some(password_hash) =
        UserCredentials::get_password_hash_by_name(&mut db, &login_data.display_name)
            .await
            .map_err(|error| {
                sqlx_error_to_api_response(error, None, &template_metadata)
            })?
    else {
    ...
    let did_password_match = verify(login_data.password, &password_hash)
        .map_err(|error| {
            bcrypt_error_to_api_response(&template_metadata, error, None)
        })?;
    ...
    let cookie = UserJWT::get_by_display_name(&mut db, &login_data.display_name)
        ...
        .unwrap()
        .to_cookie()
        .map_err(|error| serde_json_error_to_api_response(&template_metadata, error, Status::InternalServerError, None))?;

    cookie_jar.add_private(cookie);
    
    match is_htmx {
        IsHTMX(true) => Ok(ApiResponse::HtmxRedirect(HtmxRedirect::to(community_uri!(community::page(_))))),
        IsHTMX(false) => Ok(ApiResponse::Redirect(Redirect::to(community_uri!(community::page(_))))),
    }
}
