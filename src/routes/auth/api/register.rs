...
use crate::models::users::schema::{
    UserCredentials, UserJWT, UserMetadata, UserTable,
};
...
pub async fn post<'r>(
    ...
    let register_data = extract_data_or_return_response(
        register_data,
        "partials/auth/register_error",
    )?;

    csrf_token.verify(&register_data.authenticity_token.to_string())?;
    ...
    let user_id =
        UserTable::create(&mut tx, &register_data.display_name).await?;
    ...
    match is_htmx {
        ...
        IsHTMX(false) => Ok(ApiResponse::Redirect(Redirect::to(
            discover_uri!(discover::discover_page(Some(true), _)),
        ))),
    }
}
