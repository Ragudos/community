...
use crate::controllers::errors::create_community::{
    get_community_info_or_return_validation_error, render_error,
};
...
pub async fn post<'r>(
    ...
    let community_info =
        get_community_info_or_return_validation_error(&template_metadata, community_info)?;
    ...
    if UserTable::does_own_community(&mut db, &user_uid)
        ...
        .map_err(|error| {
            sqlx_error_to_api_response(
                error,
                Some("Failed to create community. Please try again later"),
                &template_metadata,
            )
        })?
    {
        return Err(render_error(
            &template_metadata,
            Status::from_code(508).unwrap_or(Status::Unauthorized),
            Some("You already own a community".to_string()),
            None,
            None,
        ));
    }
